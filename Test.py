...
def TestSimpleFlash():
    ...
    print("thAdmin#########", thAdmin.__dict__)
    providers = thAdmin.GetAvThermoProviderNames()
    print("# List of the names of the available thermo providers", providers)
    provider = providers[0]     # Should be Virt Mat
    thCase = 'myTh'
    #thCase = 1 # added by prabhat
    print("thermo is ThermoInterface object defind in VirtualMaterials")
    thermo = thAdmin.AddPkgFromName(provider, thCase, pkgName)
    print("thermo from TestSimpleFlash with argument 10", thermo)
    for i in cmpNames:
...
TEST_MAP = {
             10: ('TestSimpleFlash', 'Test to a flash'),
...
def RunTest(testNu=None):
  if testNu == -1:
      tests = TEST_MAP.keys()
      tests.sort()
      for test in tests:
          try:
              exec(TEST_MAP[test][0] + '()')
          except:
...
#TestMixAndFlash()
#TestLiqLiqEx()
#TestHeater() #TypeError: object of type 'NoneType' has no len()
#TestHeatEx() # Same error as above
#TestFlowsh1() # Same error as above
#TestFlowsh2() # same error as above at same place method in Dapl_vmg needs to be defined
#TestRecycle() # same error as above at same place method in Dapl_vmg needs to be defined
