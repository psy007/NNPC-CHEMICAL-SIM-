...
Classes:
ThermoInterface -- Main class of the interface
...
#try:
#from sim.thermo import Dapl_vmg
from sim.Dapl_vmg import * # changed by psy
# except:
#     raise AssertionError


#from ThermoAdmin import FlashResults, EnvelopeResults, ThermoCase
#from ThermoAdmin import ThermoCase
import ThermoAdmin
...
class ThermoInterface(object):
    ...
    def __init__(self):
        ...
        global initCount
        if initCount == 0:
            Dapl_vmg.InitializePkg()
            pass
        initCount += 1
        self.gPkgHandles = {}  # Values are tuples with handle,
        # proppkg creation string and the thermoCase object
    ...
    def _CreateAStoreThCaseObj(self, thName):
        ...
        try:
            response = Dapl_vmg.Oil(hnd, 'Store', '')
            if response[1] != '' and response[0] == 0:
    ...
    def _CreateAVMGThCase(self, thName, storeObj):
        ...
        if isinstance(storeObj, tuple):
            ...
            if ver >= 2.0:
                ...
                self.AddPkgFromName(thName, pkgName)
                hnd = self.gPkgHandles[thName][0]
                AddCompound = Dapl_vmg.AddCompound
                ...
                for ipMatrName, ipPaneDict in ipVals.items():
                    ...
                    for paneName, lstOfLsts in ipPaneDict.items():
                        if paneName in paneNames:
                            ...
                            #print 'before'
                            Dapl_vmg.SetBinaryPairValues(hnd, ipMatrName, paneIdx, vals)
                            #print 'after'
                            #for i in range(nuCmps):
                                #cmpName1 = cmps[i]
                                #for j in range(nuCmps):
                                    ...
                                    #val = lstOfLsts[i][j]
                                        ...
                                        #Dapl_vmg.SetBinaryPairValue(hnd, ipMatrName, i, j, paneIdx, val)
                    Dapl_vmg.ResetAijChanged(hnd)
                    ...
                    for paneName, lstOfLsts in ipPaneDict.items():
                        if paneName in paneNames:
                            ...
                            if kijArray != []:
                                ...
                                Dapl_vmg.SetBinaryPairValues(hnd, ipMatrName, paneIdx, kijArray)
            ...
            try:
                if hasattr(storeObj, 'oilData'):
                    ...
                    if not data: 
                        data = ''
                    response = Dapl_vmg.Oil(hnd, 'Recall', data)
            except:
                pass
    ...
    def _SetIPValFromIndex(self, val, cmpIdx2):
        ...
        Dapl_vmg.SetBinaryPairValue(self._hnd, self._ipMatrName, self._cmpIdx1, cmpIdx2, self._paneIdx, val)
        #Dapl_vmg.ResetAijChanged(self._hnd)
    ...
    def SetParent(self, parent):
        ...
        if self.parent == parent:   #self.parent != parent: psy
            for thCaseName, obj in self.gPkgHandles.items():
                thCaseObj = ThermoAdmin.ThermoCase(parent, self.name, thCaseName, obj[1])
    ...
    def DeleteObject(self, obj):
        if isinstance(obj, ThermoAdmin.ThermoCase):
    ...
    def AddPkgFromName(self, thName, pkgName):
        ...
        if nuPkgs > 3:
            hnd = Dapl_vmg.AddAggregatePkgFromName(pkgName)            
        ...
        #Same prop pkg for liq and vap
            ...
            hnd = Dapl_vmg.AddPkgFromName(pkgSplit[0]) # It seems important it may be vmgthermo instancePSY
            print("PRabhat singh yadav12365", hnd)
        thCaseObj = None
        if self.parent:
            thCaseObj = ThermoAdmin.ThermoCase(self.parent, self.name, thName, pkgName)
            self.gPkgHandles[thName] = (hnd, pkgName, thCaseObj) # This is setting a dict with tuple
        # thCaseObj = ThermoAdmin.ThermoCase(self.parent, self.name, thName, pkgName)
        # self.gPkgHandles[thName] = (hnd, pkgName, thCaseObj) # This is setting a dict with tuple
        print(self.gPkgHandles[thName])
        #Init flash settings with default values
        d = self.flashSettingsInfoDict
        myDict = {}
        for i in d:
            myDict[i] = d[i].defValue
    ...
    def ReplacePkgFromName(self, thName, pkgName):
        ...
        if ' ' in pkgName and (pkgSplit[0] not in pkgSplit[1:]):
            hnd = Dapl_vmg.ReplaceAggregatePkgFromName(pkgName, hnd)
        else:
            hnd = Dapl_vmg.ReplacePkgFromName(pkgSplit[0], hnd)
    ...
    def DeleteThermoCase(self, thName):
        ...
        if self.gPkgHandles.has_key(thName):
            Dapl_vmg.DeletePkg(self.gPkgHandles[thName][0])
...
##IP methods ######################################################################################
    def GetIPMatrixNames(self, thName):
        ...
        ipInfo = string.strip(Dapl_vmg.GetInteractionParameterMatrixNames(self.gPkgHandles[thName][0]))
    ...
    def GetNuIPPanes(self, thName, ipMatrName):
        ...
        ipInfo = string.split(Dapl_vmg.GetInteractionParameterMatrixNames(self.gPkgHandles[thName][0]))
    ...
    def GetIPValues(self, thName, ipMatrName, pane):
        ...
        try:
            hnd = self.gPkgHandles[thName][0]
            vals = Dapl_vmg.GetBinaryPairValues(hnd, ipMatrName, pane)
            return vals
        except:
            return None
    ...
    def GetIPValue(self, thName, ipMatrName, cmpName1, cmpName2, pane):
        ...
        try:
            ...
            hnd = self.gPkgHandles[thName][0]
            val = Dapl_vmg.GetBinaryPairValue(hnd, ipMatrName, idx1, idx2, pane)
            return val
        except:
            return None
    ...
    def SetIPValue(self, thName, ipMatrName, cmpName1, cmpName2, pane, value):
        ...
        try:
            ...
            idx2 = cmpNames.index(cmpName2)
            hnd = self.gPkgHandles[thName][0]
            Dapl_vmg.SetBinaryPairValue(hnd, ipMatrName, idx1, idx2, pane, value)
            Dapl_vmg.ResetAijChanged(hnd)
        except:
            try:
    ...
    def GetIPInfo(self, thName, ipMatrName, cmpName1, cmpName2):
        ...
        idx2 = cmpNames.index(cmpName2)
        hnd = self.gPkgHandles[thName][0]
        val = Dapl_vmg.GetBinaryPairInformation(hnd, ipMatrName, idx1, idx2)
        return val
...
##Compound methods################################################################################
    ...
    def AddCompound(self, thName, cmp):
        ...
        hnd = self.gPkgHandles[thName][1]
        Dapl_vmg.AddCompound(hnd, cmp)   # hnd is thermocase object like PR may be
    ...
    def AddHypoCompound(self, thName, hypoName, hypoDesc):
        ...
        dblVals = hypoDesc[5]        
        Dapl_vmg.AddGeneralCompound(hnd, strDescs, strVals, lngDescs, lngVals, dblDescs, dblVals)
    ...
    def EditCompound(self, thName, cmpIdx, hypoDesc):
        ...
        dblDescs = CompoundPropNameFromSimToVmg('Double', hypoDesc[4])
        dblVals = hypoDesc[5]
        Dapl_vmg.EditGeneralCompound(hnd, cmpIdx, strDescs, strVals, lngDescs, lngVals, dblDescs, dblVals)


    def DeleteCompound(self, thName, cmp):
        """Removes a compound from a thermo case"""        
        hnd = self.gPkgHandles[thName][0]
        idx = Dapl_vmg.CompoundIndexFromName(hnd, cmp)
        Dapl_vmg.DeleteCompound(hnd, idx)

    def GetSelectedCompoundNames(self, thName):
        """List of selected compounds for a thermo case"""        
        hnd = self.gPkgHandles[thName][0]

        ucmps = Dapl_vmg.SelectedCompoundNames(hnd)
        #ucmps = ["PROPANE", "n-BUTANE", "ISOBUTANE", "n-PENTANE"]
    ...
    def GetCompoundProperties(self, thName, cmpName, propNames):
        ...
        if type('s') == type(propNames):
            ...
            if sProp in self.propHandler.GetCmpSimIDPropertyNames() and sProp != 'Id':
                try: return Dapl_vmg.CompoundStringProperty(hnd, cmpName, sProp)
                except: return None
        else:
            ...
            for prop in propNames:
                sProp += prop + ' '
        try:
            return Dapl_vmg.CompoundDoubleProperty(hnd, cmpName, sProp, propCount)
        except:
            propsOut = []
            for i in propNames:
                try: propsOut.append(Dapl_vmg.CompoundDoubleProperty(hnd, cmpName, propNames[i], 1)[0])
    ...
    def GetSelectedCompoundProperties(self, thName, cmpNo, propNames):
        ...
        if propCount == 1 and sProp in GetSimHypoStrings():
            ...
            strProp = Dapl_vmg.SelectedCompoundStringProperty(hnd, cmpNo, sProp)
            return [strProp]
        else:
            # get one or more double properties
            return Dapl_vmg.SelectedCompoundDoubleProperty(hnd, cmpNo, sProp, propCount)
    ...
    def ExchangeCompound(self, thName, cmp1Name, cmp2Name):
        ...
        if idx1 != None and idx2 != None:
            Dapl_vmg.ExchangeCompound(hnd, idx1, idx2)
    ...
    def MoveCompound(self, thName, cmp1Name, cmp2Name):
        ...
        if cmp2Name == '$':
            ...
            lng = len(cmps)
            Dapl_vmg.MoveCompound(hnd, idx1, lng-1)
            Dapl_vmg.ExchangeCompound(hnd, lng-1, lng-2)
        else:
            idx2 = cmps.index(cmp2Name)
            Dapl_vmg.MoveCompound(hnd, idx1, idx2)
        return None
...
##Stream methods ######################################################################################
    ...
    def GetSpecialProperty(self, thName, inputData, frac, prop, nuPoints=None):
        ...
        if feed == None:
            feed = Dapl_vmg.RegisterObject(hnd, 'feed')
        ...
        if vmgProp in ("BOILINGCURVE", "PROPERTYTABLE"):
            ...
            Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac)
            if vmgProp == "BOILINGCURVE":
                outputData, status = Dapl_vmg.GetSpecialProperty(hnd, feed, vmgProp)
            else:
                outputData, status = Dapl_vmg.GetPropertyTable(hnd, feed, vmgProp, nuPoints)
...
##        feed = Dapl_vmg.RegisterObject(hnd, 'feed')
        ...
        if (phase == OVERALL_PHASE) or \
           ...
           (not P_VAR in (inProp1[0], inProp2[0])):
            needsFlash = 1
##        if needsFlash:
##            vap = Dapl_vmg.RegisterObject(hnd, 'vap')
##            liq0 = Dapl_vmg.RegisterObject(hnd, 'liq0')
##            liq1 = Dapl_vmg.RegisterObject(hnd, 'liq1')
        ...
        if type(prop1) != type(Numeric.array(())):
            Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, (prop1Type, prop2Type, seaPhaseType),
                                              (prop1, prop2, phase))

            Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac)
            if not needsFlash:
                values = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, feed, propIDs)
                ...
                phasesFracs = Dapl_vmg.EquilibriumObjectFlash(hnd, 0, flType, liqPhases, 0, 0, feed, phasesOut, phasesFracs)
                if phase == OVERALL_PHASE:
                    values =  Dapl_vmg.MultipleBulkObjectDoubleValues(hnd, liqPhases, 0, 0, feed, phasesOut, phasesFracs, propIDs, seaSeapp)
                elif phase == VAPOUR_PHASE: 
                    values = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, vap, propIDs)
                elif phase == LIQUID_PHASE: 
                    values = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, liq0, propIDs)
            ...
            if not needsFlash:
                ...
                for i in range(len(prop1)):
                    ...
                    Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac[i])
                    if not needsFlash:
                        valueArrays.append(Dapl_vmg.GetMultipleObjectDoubleValues(hnd, feed, propIDs))
                    else:
                        #Do a flash calculation
                        phasesFracs = Dapl_vmg.EquilibriumObjectFlash(hnd, 0, flType, liqPhases, 0, 0, feed, phasesOut, phasesFracs)
                        if phase[i] == OVERALL_PHASE:
                            valueArrays.append(Dapl_vmg.MultipleBulkObjectDoubleValues(hnd, liqPhases, 0, 0, feed, phasesOut, phasesFracs, propIDs, seaSeapp))
                        elif phase[i] == VAPOUR_PHASE: 
                            valueArrays.append(Dapl_vmg.GetMultipleObjectDoubleValues(hnd, vap, propIDs))
                        elif phase[i] == LIQUID_PHASE: 
                            valueArrays.append(Dapl_vmg.GetMultipleObjectDoubleValues(hnd, liq0, propIDs))
    ...
    def GetArrayProperty(self, thName, inProp1, inProp2, phase, frac, property):
        ...
        if type(prop1) != type(Numeric.array(())):
            Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed,
                                              (prop1Type, prop2Type, seaPhaseType),
                                              (prop1, prop2, phase))

            if propID == seaComposition:
                Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaStdLiqVolFraction, frac)
            else:
                Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac)

            values = Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, propID, 0)
        else:
            #valueArrays = []
            #if 1:
            self._hnd = hnd
            ...
            valueArrays = map(self._ArrPropertiesForIter, prop1, prop2, phase, frac)
                ...
                #for i in range(len(prop1)):
                    #Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed,
                                                      ...
                                                      #(prop1[i], prop2[i], phase[i]))
                    #Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac[i])
                    #valueArrays.append(Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, propID, 0))
...
##        feed = Dapl_vmg.RegisterObject(hnd, 'feed')
        ...
        if type(t) != type(Numeric.array(())):
            Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, (seaTemperature, seaPressure, seaPhaseType), (t, p, seaVapourPhase))
            kvalues = Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, seaIdealKValue, 0)
        else:
            kArrays = []
            for i in range(len(t)):
                Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, (seaTemperature, seaPressure, seaPhaseType),
                                                  (t[i], p[i], seaVapourPhase))
                kArrays.append(Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, seaIdealKValue, 0))
                
            kvalues = Numeric.array(kArrays)
       
##        Dapl_vmg.UnregisterObject(hnd, feed)
        return kvalues
    
    def GetMolecularWeightValues(self, thName):
        """shortcut method to get the molecular weight vector of a thermo case"""
        
        hnd = self.gPkgHandles[thName][1] # made changes 1 instead of 0
        global glbVmgObjects
        feed = glbVmgObjects.get((hnd, 'feed'), None)
        if feed == None:
            feed = Dapl_vmg.RegisterObject(hnd, 'feed')
            glbVmgObjects[(hnd, 'feed')] = feed
        try:
            return Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, seaMolecularWeightArray, 0)
        except:
            return None
        
    
    #def _CmpMWForIter(self, cmpNo):
        #return Dapl_vmg.SelectedCompoundDoubleProperty(self._hnd, cmpNo, 'MolecularWeight', 1)
    
    
    def _PropertiesForIter(self, prop1, prop2, phase, frac):
        feed = self._feed
        hnd = self._hnd
        Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, (self._prop1Type, self._prop2Type, seaPhaseType), (prop1, prop2, phase))
        Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac)
        return  Dapl_vmg.GetMultipleObjectDoubleValues(hnd, feed, self._propIDs)
    ...
    def _ArrPropertiesForIter(self, prop1, prop2, phase, frac):
        ...
        Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, (self._prop1Type, self._prop2Type, seaPhaseType), (prop1, prop2, phase))
        Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, frac)
        return Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, self._propID, 0)    
...
##Flash methods ####################################################################################
    ...
    def SetFlashSetting(self, thName, settingName, value):
        ...
        Dapl_vmg.SetFlashSetting(hnd, vmgSettingName, myVal)
    ...
    def Flash(self, thName, cmps, properties, liqPhCount,
        ...
        print("Verifying Vars",vars,phasesFracs)

        vals = []
        for i in vars:
            vals.append(properties[i].GetValue())

        vmprops = self.propHandler.PropNamesFromSimToVmg(vars)
        hnd = self.gPkgHandles[thName][1]  # this is holding "PR"

        feed = glbVmgObjects.get((hnd, 'feed'), None)
        if feed == None:
            feed = Dapl_vmg.RegisterObject(hnd, 'feed')
            print("content of feed",feed)
            glbVmgObjects[(hnd, 'feed')] = feed
        ...
        if withSolid:  # VMG uses the solid as the first phase
            phasesOut.insert(0, solidId)
            if nuSolids > 1:
                thThermoAdmin.InfoMessage('TooManySolidPhases', (nuSolids, thName))
                
        bulkComp = cmps.GetValues()
        Dapl_vmg.SetObjectDoubleArrayValues(hnd, feed, seaComposition, bulkComp)
        Dapl_vmg.SetMultipleObjectDoubleValues(hnd, feed, vmprops, vals)
        try:
            phasesFracs = Dapl_vmg.EquilibriumObjectFlash(hnd, initFromInput, flType, liqPhases, withSolid,
                                                          othersCount, feed, phasesOut, phasesFracs)
            #phasesFracs = [np.asarray([0.3,0.3,0.25,0.25]),np.asarray([0.3,0.3,0.25,0.25]),np.asarray([0.3,0.3,0.25,0.25])]
        except Dapl_vmg.VMGwarning, e:
        ...
        if not propList:
            # propNames is the property name which vmg undersatnd we need to replace
            # it which is used by Ollin seaXXXX needs to be changed
            propsNames = self.propHandler.GetVmgCommonPropertyNames()
            #print("Propertyname", propsNames)
        ...
        if seaStdLiqVolume in propsNames:
            doStdVol = True
        else:
            doStdVol = False
        if seaStdLiqDensity in propsNames:
            doStdDen = True
        else:
            doStdDen = False
        ...
        bulkProps = Dapl_vmg.MultipleBulkObjectDoubleValues(hnd, liqPhases, withSolid, othersCount, feed, phasesOut,
                                                       phasesFracs, propsNames, seaSeapp) # Get bulk values

        bulkArrProps = []

        if withSolid:  # sim42 uses the solid as the last phase
            phasesFracs = list(phasesFracs) 
            temp = phasesFracs.pop(0)
            phasesFracs.append(temp)
            temp = phasesOut.pop(0)
        ...
        for prop in arrPropNames:  # arrPropNames coming from VMconstant.py
            #print("prop",prop)
            #vals = Dapl_vmg.GetObjectDoubleArrayValues(hnd, feed, prop, -1)
            #print("vals",vals)
            bulkArrProps.append(vals)

        keepValue = -1
        #liqComposit = Dapl_vmg.GetObjectDoubleArrayValues(hnd, liq1Id, seaComposition, keepValue)
        if liqPhCount == 1:
           if phasesFracs[2] != 0.0:
               ...
               Dapl_vmg.MixTwoObjects(hnd, bulkLiqId, liq1Id, liq2Id, fr1, propsNames)  # MixTwoObjects is not yet
                                                                                        # implemented
               ...
               phasesOut.remove(liq1Id)  # fixup phasesOut : remove the liq1 and liq2
        ...
        # keepValue = -1
        for i in phasesOut:
            composit = Dapl_vmg.GetObjectDoubleArrayValues(hnd, i, seaComposition, keepValue)
            phasesComposit.append(composit)
            lstArrProps = []

            for prop in arrPropNames:
                vals = Dapl_vmg.GetObjectDoubleArrayValues(hnd, i, prop, keepValue)
                lstArrProps.append(vals)

            phasesArrProps.append(lstArrProps)
            props = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, i, propsNames)  # Phase props
        ...
        #         #     if not stdVolRefT:
        #         #         stdVolRefT = 288.15
        #         #     Dapl_vmg.SetMultipleObjectDoubleValues(hnd, liq1Id, [seaTemperature], [stdVolRefT])
        #         #     Dapl_vmg.SetObjectDoubleArrayValues(hnd, liq1Id, seaComposition, bulkComp)
        #         #
        #         #     stdVol, stdDen = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, liq1Id, [seaStdLiqVolume, seaStdLiqDensity])
        #         #
        #         #     if doStdVol:
        ...
        #         #         Dapl_vmg.SetObjectDoubleArrayValues(hnd, liq1Id, seaComposition, phasesComposit[i])
        #         #         stdVol, stdDen = Dapl_vmg.GetMultipleObjectDoubleValues(hnd, liq1Id, [seaStdLiqVolume, seaStdLiqDensity])
        #         #         if doStdVol:
        ...
        return ThermoAdmin.FlashResults(propsNamesOut, arrPropNamesOut, bulkComp, bulkProps, bulkArrProps,
    ...
    def PhaseEnvelope(self, thName, cmps, vapFraction, pressure, maxPoints, pList=None):
        ...
        if feedId == None:
            feedId = Dapl_vmg.RegisterObject(hnd, 'feed')
            glbVmgObjects[(hnd, 'feed')] = feedId
            
##        hnd = self.gPkgHandles[thName][0]
##        feedId = Dapl_vmg.RegisterObject(hnd, 'feed')
        #bulkComp = cmps.GetValues()
        Dapl_vmg.DefineObject(hnd, feedId, seaLiquidPhase, 298.15, pressure, cmps, seaSeapp)
        pCount = 0
        if (pList): pCount = len(pList)
        nc = len(cmps)
        try:
            results = Dapl_vmg.PhaseEnvelope(hnd, feedId, pCount, pList, vapFraction, nc, maxPoints)
        except:
            # continue the calculation when failed
            pass
##        Dapl_vmg.UnregisterObject(hnd, feedId)
        ...
        for i in range(npt):
            ...
            i1 = i2
        return ThermoAdmin.EnvelopeResults(retCode, msg, npt, types, pVals, tVals, kVals)
...
##Save and load methods ############################################################################
    ...
    def _BuildThermoXMLString(self, gPkgHandles):
        ...
        for thCaseName in gPkgHandles.keys():
            ...
            cmpList = Dapl_vmg.SelectedCompoundNames(gPkgHandles[thCaseName][0])
        ...
        def startElement(self, name, attrs):
            if name == 'ThermoProvider':
                ...
                if self.pkgSet:
                    ...
                    Dapl_vmg.AddCompound(self.hnds[self.temphndName][0], cmp)
        ...
        def endElement(self, name):
            if name == 'PropertyPackages':
                ...
                if ' ' in self.propPkg and (tst[0] not in tst[1:]):
                    handle = Dapl_vmg.AddAggregatePkgFromName(self.propPkg)
                else:
                    handle = Dapl_vmg.AddPkgFromName(tst[0])
...
##Oil Methods ######################################################################################
    ...
    def _VMGCommand(self, hnd, key, cmd):
        ...
        result = Dapl_vmg.CustomCommand(hnd, key, cmd)
