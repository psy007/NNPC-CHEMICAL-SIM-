...
seaComposition ="Ki" # -4 # randomly changed
seaPhaseType = -3
seaTemperature = "T"  # 0
seaPressure = "P"  # -1
seaMolecularWeight = "Mol_WT"  # 0
seaZFactor = "Z"  # 1
seaVolume = 2
seaMassDensity = 3
seaLnFugacityCoefficient = "Ki" # 4 randomly changed
seaLnActivityCoefficient = "Ki" # 5 randomly changed
seaLnStandardStateFugacity = "Ki" # 6 randomly changed
seaLnFugacity = "Ki" # 7 randomly changed
seaEnthalpy = "H"   #8
seaEntropy = "S"    # 9
seaCp = 10
seaCv = 11
...
seaIdealGasGibbs = "Ki" # 27 randomly changed
seaMechanicalZFactor = 28
seadVdTp = 29
seaLnPoynting = 30
seaLnSatFugacity = "Ki"  # 31 randomly changed
seaLnVaporPressure = 32
seaMassFraction = "Ki" # 33 randomly changed  # mass fraction array
seaIdealVolumeFraction = "Ki" # 34 randomly changed  # ideal volume fraction array
seaGibbsFreeEnergy = "GF"  # Gibbs free energy
seaHelmholtzEnergy = "AFree"  # Helmholtz energy
seaInternalEnergy = "U"  # Internal energy
seaIdealKValue = "Ki" # 38 randomly changed   # ideal k value
seapH = 39  # pH
seaStdLiqDensity = "LiqDen"  # standard liquid density
...
seaStdLiqVolFraction = "Ki" # 61 randomly changed  # liquid volume fraction array
seaStdLiqVolumePerCmp = "Ki" # 62 randomly changed  # liquid molar volume per component
seaPseudoTc = 63  # pseudo critical temperature
seaPseudoPc = 64  # pseudo critical pressure
seaPseudoVc = 65  # pseudo critical volume
seaEnvelopeCmp = "Ki" # 66 randomly changed
seaMolecularWeightArray = "Ki" # 77 randomly changed  # Molecular weight as an array
seaMassCp = 68
seaMassCv = 69
seaMassEnthalpy = 70
seaMassEntropy = 71
...
class vmgPropHandler(object):
    ...
    def __init__(self):
        ...
        self._simCommonArrayProps = tuple(_simArrayProps)  # PSY simArrayProps
    ...
    def GetCmpSimIDPropertyNames(self):
        return _GetCmpSimIDProps()

    def GetCmpVmgIDPropertyNames(self):
        return _GetCmpVmgIDProps()

    def GetCmpSimFixedPropertyNames(self):
        return _GetCmpSimFixedProps()

    def GetCmpVmgFixedPropertyNames(self):
        return _GetCmpVmgFixedProps()

    def GetCmpSimEqDepPropertyNames(self):
        return _GetCmpSimEqDepProps()

    def GetCmpVmgEqDepPropertyNames(self):
        return _GetCmpVmgEqDepProps()
    ...
    def GetSimPropertyNames(self):
        return list(_simProps)  # Send a copy, not the original one

    def GetVmgPropertyNames(self):
        return list(_vmgProps)  # Send a copy, not the original one
    ...
    def GetSimArrayPropertyNames(self):
        return list(_simArrayProps)

    def GetVmgArrayPropertyNames(self):
        return list(_vmgArrayProps)
    ...
    def GetVmgCommonArrayPropertyNames(self):
        ...
        return tuple(_vmgArrayProps)        # changed by psy uncomment above

    def GetSimCommonArrayPropertyNames(self):
        """Returns the tuple of simCommonProperty Names"""
        #return self._simCommonArrayProps
        return tuple(_simArrayProps)   # changed by psy uncomment above
...
class vmgFlashSettingsInfoDict(dict):
    ...
    def __init__(self):
        ...
        self["Retrograde calculations"] = FlashSettingInfo(seaIsRetrograde, "Retrograde calculations", 0,
                                                           options=[0, 1])
        self["Vapor frac spec"] = FlashSettingInfo(seaVapFracSpec, "Vapor frac spec", vapFracOpts[0],
                                                   options=vapFracOpts)
