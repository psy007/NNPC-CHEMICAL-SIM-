...
Remarks:
This module provides a class with a standard interface to available thermo
providers. 
...
from VirtualMaterials import ThermoInterface
...
class ThermoDict(UserDict):
    """Dictionary implemented to handle the thermo interfaces classes

    keys -- Name of the provider (Name of the file with the class)
    values -- Instance of the thermo interface of the provider

    """
    def __init__(self, dict=None):
        """Init an empty dictionary"""
        print("## I am in ThermoDict() cons")
...
class ThermoAdmin(object):
    ...
    def __init__(self):
        ...
        print(" ## I am in ThermoAdmin() cons")
        self.thDict = ThermoDict()  # Modified by AR  {VMModName: VMClassName} # remodified by prabhaat
        print(self.thDict)
        ...
        err = self.SetNewThermoProvider(VMModName, VMClassName)  # Commented by AR because err is not used anywhere
        self.currTypeOfCmpID = 'VMName'     # Could be VM Id, CASN, DIPPR ID, etc.
        print(self)

    def CleanUp(self):
        for i in self.thDict.values():
            i.CleanUp()
        self._linkedUOs = []
...
##Methods not implemented by thermo providers ###############################################
    ...
    def GetAvThermoProviderNames(self):
        ...
        #print("PPPPPPPPPPPPP",self.thDict.keys())
...
####################################################################################################
    ...
    def AddPkgFromName(self, provider, thName, pkgName):
        ...
        provider -- Name of the thermo provider 'VirtualMaterial'
        thName -- Name of the thermo case 'myTh'
        pkgName -- String with the th pkg name. If one pkg per phase,
                   then separate the th pkg names with a space. Order: Vap, liq
        """

        # Add pkg
        #if self.thDict[provider] == 'ThermoInterface':

        thCaseObj = self.thDict[provider].AddPkgFromName(thName, pkgName)
        # thCaseObj = ThermoInterface().AddPkgFromName(thName, pkgName)
        #exec (TEST_MAP[testNu][0] + '()')
        # thermo_interface_obj = ThermoInterface()
        #
        # thCaseObj = thermo_interface_obj.AddPkgFromName(thName, pkgName)
...
##Compound methods################################################################################
    ...
    def AddCompound(self, provider, thName, cmp):
        ...
        selCmps = self.GetSelectedCompoundNames(provider, thName) # this is giving problem
        #selCmps =["PROPANE", "n-BUTANE"] #get selected compound will retun selected cmpd from ollin
    ...
    def GetSelectedCompoundNames(self, provider, thName):
        ...
        # uncomment above line will give 'str' object has no attribute 'GetSelectedCompoundNames'
