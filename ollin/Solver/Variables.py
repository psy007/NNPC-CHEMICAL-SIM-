...
import numpy6
from numpy import float64,  array, zeros, ones, sum
import S42Glob
...
class BasicProperty(object):
    ...
    def CheckTolerance(self, value):
        ...
        #print("Type checking",type(tolerance), type(error), error)
        try: # try except defined by prabhat
            if error > tolerance:
...
class CompoundList(list):
    ...
    def SetValues(self, vals, calcStatus):
        ...
        try:
            ...
            vals = array(map(float, vals), float64)
            total = sum(vals)
    ...
    def AreValuesReady(self):
        ...
        try:
            ...
            values = array(self.GetValues(), float64)
            return 1
        except:
            return 0
    ...
    def SumFractions(self):
        ...
        """
        try:
            return sum(array(self.GetValues(), float64))
        except:
            return None
    ...
    def SetValues(self, values, calcStatus=CALCULATED_V):
        ...
        try:
            ...
            try:
                ...
                values = array(map(float, values), float64) / mw
...
class StdVolCompoundList(object):
    ...
    def SetValues(self, values, calcStatus=CALCULATED_V):
        ...
        try:
            ...
            thCaseObj = unitOp.GetThermo()
            thAdmin, prov, case = thCaseObj.thermoAdmin, thCaseObj.provider, thCaseObj.case
            values = array(map(float, values), float64)
            values = values/sum(values)
